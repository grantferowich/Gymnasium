...
function latticePaths(x, y) {
  ...
  function traverse(x, y) {
    let key = x + '_' + y;
    if(cache[key]) return cache[key];  //STEP 2
    if(x === 0 && y === 0) return 1;
    if(x < 0 || y < 0) return 0;
    let result = traverse(x - 1, y) + traverse(x, y - 1);
    cache[key] = result;  //STEP 3
    return result;
  }
  return traverse(x, y);
}
  //tabulation approach
function latticePathsTabulation(rowSize, colSize) {
  //create the first array that will become the table
  let table = new Array(rowSize + 1).fill(1);
  //i represents "height" or amount of iterations of the row
  for (let i = 0; i < colSize; i++) {
  //iteratation of the row
