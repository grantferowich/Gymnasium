...
func (matrix *Matrix) print() {
  for row := 0; row < matrix.m; row++ {
      for col := 0; col < matrix.n; col++ {
        fmt.Print(strconv.Itoa(matrix.storage[row][col]) + "\t")
      }
      fmt.Println()
  }
}
...
func (matrix *Matrix) isValid(i, j int) bool {
  return i >= 0 && j >= 0 && i < matrix.m && j < matrix.n
}

func (matrix *Matrix) reinitialize(arrayOfArrays [][]int) {
  matrix.storage = arrayOfArrays
  matrix.m = len(arrayOfArrays)
  if matrix.m > 0 {
    matrix.n = len(arrayOfArrays[0])
  }
}
...
func (matrix *Matrix) scale(factor int) {
  ...
  for i := 0; i < matrix.m; i++ {
      for j := 0; j < matrix.n; j++ {
          newVal = matrix.retrieve(i, j) * factor
          matrix.insert(i, j, newVal)
      }
  }
}
...
func (matrix *Matrix) flatten() []int {
  var result []int = make([]int, matrix.m*matrix.n)
  counter := 0
  for i := 0; i < matrix.m; i++ {
      for j := 0; j < matrix.n; j++ {
          result[counter] = matrix.storage[i][j]
          counter++
      }
  }
  return result
...
func (matrix *Matrix) slice(rowRange, colRange []int) *Matrix {
  ...
  rowMax := min(rowRange[1], matrix.m)
  colMin := max(0, colRange[0])
  colMax := min(colRange[1], matrix.n)

  newMatrix := new(Matrix).Init(rowMax - rowMin, colMax - colMin)
  var newVal int
  for i := rowMin; i < rowMax; i++ {
      for j := colMin; j < colMax; j++ {
          newVal = matrix.retrieve(i, j)
...
func (matrix *Matrix) transpose() *Matrix {
  newMatrix := new(Matrix).Init(matrix.n, matrix.m)
  var val int
  for i := 0; i < matrix.m; i++ {
      for j := 0; j < matrix.n; j++ {
          val = matrix.retrieve(i, j)
...
func (matrix *Matrix) multiply(matrix2 *Matrix) *Matrix{
  ...
  newMatrix := new(Matrix).Init(matrix.m, matrix2.n)
  var newVal int
  for i := 0; i < newMatrix.m; i++ {
      for j := 0; j < newMatrix.n; j++ {
          newVal = 0
          for k := 0; k < matrix.n; k++ {
              newVal += matrix.retrieve(i, k) * matrix2.retrieve(k, j)
          }
