...
func (graph *Graph) Init() {
  graph.storage = make(map[int][]int)
}

//   Time Complexity: O(1)
//   Auxiliary Space Complexity: O(1)
func (graph *Graph) addVertex(id int) bool {
  if mapContainsKey(graph.storage, id) {
    return false
  }

  graph.storage[id] = []int{}
  return true
}
...
func (graph *Graph) removeVertex(id int) bool {
  ...
  for key, edges := range graph.storage {
    _ = key
    var index int = findIndex(edges, id)
    if index > -1 {
      graph.storage[key] = removeIndex(edges, index)
    }
  }
  delete(graph.storage, id)
  return true
}
...
func (graph *Graph) addEdge(id1 int, id2 int) bool {
  if !mapContainsKey(graph.storage, id1) || !mapContainsKey(graph.storage, id2) {
    return false
  }

  if(findIndex(graph.storage[id1],id2) >= 0) { return false }
  graph.storage[id1] = append(graph.storage[id1], id2)
  return true
}
...
func (graph *Graph) removeEdge(id1 int, id2 int) bool {
  if mapContainsKey(graph.storage, id1) && mapContainsKey(graph.storage, id2) {
    ...
    var index int = findIndex(graph.storage[id1], id2)

    if index >= 0 {
      graph.storage[id1] = removeIndex(graph.storage[id1], index)
      return true
    }
  }
  return false
...
func (graph *Graph) isVertex(id int) bool {
  return mapContainsKey(graph.storage, id)
}
