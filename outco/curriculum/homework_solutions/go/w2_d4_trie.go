...
func (trie *Trie) insert(word string) bool {
  ...
  for _, letter:= range word {
    if childNode, ok := currentNode.children[letter]; ok {
      currentNode = childNode
    } else {
      currentNode.children[letter] = new(TrieNode).Init(letter)
...
func (trie *Trie) _search(word string) *TrieNode {
    var currentNode *TrieNode = trie.root
    for _, letter:= range word {
      if childNode, ok := currentNode.children[letter]; ok {
        currentNode = childNode
...
func (trie *Trie) _dfs(currentNode *TrieNode, buildStr string, results *[]string) {
  if(currentNode == nil) { return }
  if(currentNode.endOfWord) {
    *results = append(*results, buildStr)
  }
  for childChar, childNode := range currentNode.children {
    trie._dfs(childNode, buildStr + string(childChar),results)
  }
}
...
func (trie *Trie) isWord(word string) bool {
  ...
  var terminalNode *TrieNode = trie._search(word)
  return terminalNode != nil && terminalNode.endOfWord
}

func (trie *Trie) startsWith(prefix string) []string {
  var currentNode *TrieNode = trie._search(prefix);
  if(currentNode == nil) { return nil }
  results := []string{}
  trie._dfs(currentNode, prefix, &results)
  return results
}
...
func (trie *Trie) remove(word string) {
  ...
  if(len(currentNode.children) > 0) { return }

  var previousLetter rune
  for !currentNode.endOfWord && len(stack) > 0 {
    previousLetter = currentNode.value
    currentNode = stack[len(stack)-1]
    stack = stack[:len(stack)-1]
    delete(currentNode.children, previousLetter)
  }
}
