...
func traverse(current *Vertex, path *[]int){
  if visited[current] { return }
  *path = append(*path, current.id)
  visited[current] = true
  if current.id == destID {
    result = append(result, copySlice(*path))
  } else {
    for _,edgeVertex := range current.edges {
       traverse(edgeVertex, path)
    }
  }
