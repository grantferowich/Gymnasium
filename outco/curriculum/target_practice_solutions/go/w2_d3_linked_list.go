...
// Helper function to locate a ListNode and its parent
func _locate(node *ListNode, target int) []*ListNode {
  var prev *ListNode = nil
  var current *ListNode = node
  var results []*ListNode
  for current != nil {
    if current.value == target {
      return []*ListNode{prev, current}
    }
    prev = current
    current = current.next
  }
...
func swap(head *ListNode, a int, b int) *ListNode {
  ...
  var location1 []*ListNode = _locate(head, a)
  var location2 []*ListNode = _locate(head, b)
  var prev1 *ListNode = location1[0]
  var current1 *ListNode = location1[1]
  var prev2 *ListNode = location2[0]
  var current2 *ListNode = location2[1]

  // If a/b not found no swap
  if current1 == nil || current2 == nil { return head }
  ...
  // swap next of `currents`
  var temp *ListNode = current1.next
  current1.next = current2.next
  current2.next = temp

  // if either current1/urrent 2 was head, need to return new head
  if prev1 == nil {
    return current2
  }
  if prev2 == nil {
  ...
  // return unchanged head if neither current1/current2 was head
  return head
}
