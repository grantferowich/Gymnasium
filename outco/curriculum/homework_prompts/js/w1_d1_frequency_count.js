...
// Can we assume valid input? No. String may be empty. 

function wordCount(sentence) {
    let wordsArray = sentence.toLowerCase().replace(/[^\w\s]/g, "").split(" ");
    ...
    for (let k = 0; k < wordsArray.length; k++){
     if (!hash[wordsArray[k]]){
      hash[wordsArray[k].toString()] = 1;
     } else {
      hash[wordsArray[k].toString()] += 1;
     }
    }
...
// edge : a string must contain r,b, and g to be valid
// do not assume valid input
// return a frequency count
// loop over the string starting at index 0
// increment count if i, i+1, i+2 concatenated is equal to "rbg"

function rgb(string) {
  // return 0 when the input string is invalid
 ...
 * 'noon' --> 'no'
 */
// replace all spaces with empty space
// loop over the string of characters
// store the character and a count of 1 if the character has not been seen
// increment the character's count if the character has been seen before

// loop through each key in the map checking if the value is greater than the maxKey
// loop through the map to validate if the maxkey's value is equal to the key's value
// append keys to a string when the key's value equals the maxKey's value  
