...
const printDFSWithRecursion = (graph, start) => {
    let visited = new Set();
    const traverse = (current) => {
        ...
        visited.add(current);
        //operate on current here
        let neighbors = graph[current];
        // console.log(neighbors)
        console.log(current)
        if (neighbors !== undefined){
            for (let x = 0; x < neighbors.length; x++){
                traverse(neighbors[x]);
            }
        }
    }
    traverse(start)
...
// const edgesY = [[1,2], [1,3], [2,3]]; // 1 2 3
const graphX = generateAdjacencyList(edgesX);
// const graphY = generateAdjacencyList(edgesY); // 1 2 3
// printDFSWithRecursion(graphY, 1)
printDFSWithRecursion(graphX, 1) // 1 2 4 8 9 5 6 7 3